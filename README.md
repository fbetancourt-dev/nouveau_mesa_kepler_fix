...
### 4. `patches/nouveau_tic_bufctx_refn.patch` (Unconditional TIC Bufctx Reference Validation)
* **Problem:** When dynamic texture buffer objects (TBOs / `PIPE_BUFFER`) or live HTML5/WebGL video streaming surfaces update their physical VRAM addresses, if a texture surface remains active but unmodified across subsequent frames (`!dirty && !need_flush`), `nve4_validate_tic` in `src/gallium/drivers/nouveau/nvc0/nvc0_tex.c` previously skipped calling `BCTX_REFN(nvc0->bufctx_3d, 3D_TEX(s, i), res, RD)`. Omitting `BCTX_REFN` meant `res->bo` was evicted from `bufctx_3d`, so the Nouveau DRM kernel driver did not map the buffer's GPU virtual memory pages in the channel's MMU table during pushbuf submission. When GNOME Shell / Wayland compositors sampled the live video texture surface (`GPC0/TPC0/TEX: 80000041`), the GPU 3D engine attempted to read from unmapped VRAM `0x26566000`, causing a `reason 02 [PTE]` fault on channel 6 and killing the GUI session.
* **Why the Fix Works:** Enforces unconditional `BCTX_REFN(nvc0->bufctx_3d, 3D_TEX(s, i), res, RD)` execution for all valid active textures in `nve4_validate_tic`. Guarantees that 100% of active video and texture surfaces remain mapped in the GPU MMU page tables across every 3D draw call, preventing unmapped VRAM `PTE` page faults.
* **Verification Test:** Executed by **TEST 2D** (dynamic `glTexBuffer` address flushes), real-time video streaming compositing, and live kernel diagnostic audit (**SECTION 4**).
...
===================================================================================================================
Timestamp: 2026-08-08 21:51:46 | System: Linux x86_64
===================================================================================================================
ID         | Category           | Check Metric                         | Status     | Details
-------------------------------------------------------------------------------------------------------------------
AUDIT-01   | Kernel Logs        | Nouveau GPU Faults (PTE/OOR_ADDR)    | PASS       | 0 Nouveau GPU traps or PTE page faults in log
AUDIT-02   | Kernel Logs        | Nouveau FIFO Reset Events            | PASS       | 0 Nouveau FIFO channel reset events detected
...
AUDIT-05   | NVIDIA Power PM    | dGPU PCI Runtime PM (nouveau.runpm)  | DISABLED   | Runtime PM Disabled (control: on)
AUDIT-06   | Intel iGPU Status  | i915 Power Control & Package C8      | ACTIVE     | Intel Iris Pro 5200 (control: on, pkg_c8 fix active)
AUDIT-07   | Thermals & Fans    | GPU & CPU Core Temperatures          | PASS       | GPU: 69.0°C | CPU Package: 66.2°C
AUDIT-08   | CPU Performance    | Clock Frequencies & Turbo Boost      | PASS       | Current Core 0: 3361.8 MHz (Max Turbo: 3700 MHz)
...
## 🏛️ Valid vs. Deprecated / Superseded Fixes Matrix
...
| **BMS CPU 800MHz Bypass** | `disable-bdprochot.service` (MSR `0x1FC` bit 0 clear) | **`ACTIVE`** ✅ | Clears hardware throttling signal caused by aftermarket battery or BMS metric mismatch, restoring full 3.70 GHz Turbo Boost. |
| **CPU Governor Strategy** | `cpu-governor-balanced.service` (`schedutil` governor) | **`ACTIVE`** ✅ | Enforces "Race to Sleep" dynamic scaling, giving 3.7 GHz performance on demand while keeping idle temperatures low (**58°C–65°C**). |
...
### 8. Check NVIDIA dGPU Always-ON Power Management Status
Check PCI power management control (`on` = locked active to prevent VRAM PTE faults & DisplayPort link failures):
```bash
# Check PCI power control state ('on' = Always-ON stability lock)
cat /sys/bus/pci/devices/0000:01:00.0/power/control

# Check real-time dGPU status ('active' = awake and stable)
...
# Verify kernel cmdline parameters for NVIDIA runpm=0 and Intel Package C8 disable (i915.enable_pkg_c8=0)
